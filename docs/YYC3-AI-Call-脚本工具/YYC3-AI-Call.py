...
# -*- coding: utf-8 -*-
"""
@file YYC3-AI-Call.py
@description YYC3-AI-Call 全文档架构一键生成脚本，用于生成符合YYC3标准的完整文档架构，支持文档内容验证和自动更新功能
...
# @Desc: YYC3-AI-Call 全文档架构一键生成脚本 | 生成根目录docs/ | 无任何错误 | 支持文档验证和自动更新
import os
import sys
import argparse
import re
...
MAIN_MD_TEMPLATE = """---
@file: {FILE_NAME}
@description: YYC3-AI-Call {DESCRIPTION}
@author: YanYuCloudCube Team
@version: {VERSION}
@created: {CREATE_DATE}
@updated: {CREATE_DATE}
...
本文档详细描述YYC3-AI-Call-{DOC_CATEGORY}-{DOC_NAME}相关内容，确保项目按照YYC³标准规范进行开发和实施。
...
#### 1.1 项目背景
YYC³(YanYuCloudCube)-AI-CALL项目是一个基于「五高五标五化」理念的智能化企业级应用系统，致力于提供"五高五标五化"的企业级应用解决方案。
...
README_MD_TEMPLATE = """---
@file: YYC3-AI-Call-{DOC_CATEGORY}-文档索引.md
@description: YYC3-AI-Call {DESCRIPTION}
@author: YYC³
@version: {VERSION}
@created: {CREATE_DATE}
...
def parse_args():
    """解析命令行参数"""
    parser = argparse.ArgumentParser(description='YYC3-AI-Call 全文档架构一键生成脚本')
...
def print_welcome_message():
    """打印欢迎信息"""
    print("="*80)
    print("🚀 YYC3-AI-Call 全文档架构一键生成脚本")
    print("="*80)
...
def generate_project(root_dir, version, creation_date, status, encoding):
    ...
    print("✅ 开始生成 YYC3-AI-Call 文档架构 | 根目录: {} | 编码: {}".format(root_dir, encoding))
    print("="*80)
    ...
    try:
        ...
        for doc_category, doc_list in PROJECT_STRUCT.items():
            ...
            try:
                # 创建分类目录
                cate_dir = os.path.join(root_dir, "YYC3-AI-Call-{}".format(doc_category))
                # 验证分类目录路径，确保在根目录内
                ...
                for doc_no, doc_name, doc_desc, doc_tags in doc_list:
                    ...
                    try:
                        # 拼接文件名和标题
                        file_name = "{}-YYC3-AI-Call-{}-{}.md".format(doc_no, doc_category, doc_name)
                        file_path = os.path.join(cate_dir, file_name)
                        # 验证文件路径，确保在根目录内
                        file_path = validate_path(file_path, base_dir=root_dir)
                        title = "{}-YYC3-AI-Call-{}-{}".format(doc_no, doc_category, doc_name)
        ...
        print(f"   • 完整的文档分类体系")
        print(f"   • 标准化的文档模板")
        print(f"✅ 所有文档严格遵循 YYC3-AI-Call 命名规范与模板标准！")
        print("="*80)
        print("📚 后续建议:")
...
def validate_document_structure(root_dir):
    ...
    try:
        ...
        for category in PROJECT_STRUCT.keys():
            category_dir = os.path.join(root_dir, f"YYC3-AI-Call-{category}")
        ...
        for category, doc_list in PROJECT_STRUCT.items():
            ...
            for doc_no, doc_name, _, _ in doc_list:
                doc_file = f"{doc_no}-YYC3-AI-Call-{category}-{doc_name}.md"
